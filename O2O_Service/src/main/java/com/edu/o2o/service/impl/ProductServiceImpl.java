...
import com.edu.o2o.dao.ProductDao;
import com.edu.o2o.dao.ProductImgDao;
...
public class ProductServiceImpl implements ProductService {
     ...
     */
    public ProductExecution addProduct(Product product, ImageHolder thumbnail, List<ImageHolder> productImageList) throws ProductOperationException, IOException {
        // 空值判断
    ...
    public ProductExecution modifyProduct(Product product, ImageHolder thumbnail, List<ImageHolder> productImgHolderList) throws ProductOperationException, IOException {
            // 空值判断
            if (product != null && product.getShop() != null && product.getShop().getShopId() != null) {
                // 给商品设置上默认属性
                product.setLastEditTime(new Date());
    ...
    private void addProductImgList(Product product, List<ImageHolder> productImgHolderList) throws IOException {
        ...
        String dest = PathUtil.getImgBasePath()+PathUtil.getShopImagePath(product.getShop().getShopId());
     ...
     */
    private void addProductImageList(Product product, List<ImageHolder> productImageHolderList) throws IOException {
     ...
     * @param thumbnail
     */
    public void addThumbnail(Product product, ImageHolder thumbnail) throws IOException {
        String dest = PathUtil.getImgBasePath()+PathUtil.getShopImagePath(product.getShop().getShopId());
        String thumaddr = ImageUtil.generateThumbnail(thumbnail.getImage(),dest,thumbnail.getImageName());
        product.setImgAddr(thumaddr);
    }
}
